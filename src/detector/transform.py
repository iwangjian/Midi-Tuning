...
class DetectionTransform(object):
    ...
    def __call__(self, context, response):
        context = '\n'.join(context)
        concat_text = context + ' [SEP] ' + response
        
        context_tokenized_dict = self.tokenizer.encode_plus(concat_text)
        input_ids, input_masks = context_tokenized_dict['input_ids'], context_tokenized_dict['attention_mask']
